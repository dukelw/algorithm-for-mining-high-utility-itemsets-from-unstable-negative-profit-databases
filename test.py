...
def ehmin_mine(
    ...
    for Uk in UL.items.values():
        ...
        if contains_same_characters(check_list, "_".join(tmp)):
            continue
...
def ehmin(k: int, δ: float):
    ...
    sorted_item_dataset = sort_transaction_items(sorted_item, dataset)
    ...
    for transaction in sorted_dataset:
        ...
        for item in tmp:
            ptwus[item] = calculate_rtwu({item}, sorted_dataset) + ptu_k
